...
if st.session_state.var_df is not None:
    ...
    # Fill null gene_name with a placeholder, then group
    df_summary = (
    st.session_state.var_df
    .assign(gene_name=st.session_state.var_df['gene_name'].fillna('unknown'))
    .groupby(['gene_name', 'output_type'])
    .head(3)
    ...
    if st.button("Ask AI to analyze results"):
        with st.spinner("Thinking..."):
            try:
                ...
                The variant:
                chromosome {variant_split[0]}
                position {variant_split[1]}
        ...
        with st.expander("AI summary", expanded=True):
            with st.chat_message('ai'):
                st.markdown(f"{st.session_state.llm_response} \
                            ...
                            This summary was generated by AI and may be incorrect."
                            )
