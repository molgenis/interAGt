...
import sys
import os
sys.path.append(os.path.dirname(__file__))  # Ensure tool_call.py is in the path
from tool_call import run_tools, get_ag_scores  # Import the functions
...
if st.session_state.var_df is not None:
    ...
    # Group by gene/output_type and take the max score per group
    df_summary = st.session_state.var_df.groupby(['gene_name', 'output_type']).agg({
    'raw_score': lambda x: max(abs(x)),
    'quantile_score': 'max',
    'biosample_name': lambda x: x.mode()[:0]  # Most common tissue for this group
    }).reset_index()
    ...
    if st.button("Ask AI to summarize"):
        with st.spinner("Thinking..."):
            try:
                # Get flat text summary of scores
                ag_scores_text = get_ag_scores(df_summary)

                # Prepare prompt for LLM
                prompt = f"""
                Analyze these AlphaGenome variant scores and provide a biological interpretation.
                Focus on the most significant effects (highest absolute scores) and their potential impact on gene regulation.
                ...
                Also consider:
                - Which genes/tissues are most affected?
                - What are the potential functional consequences?
                - Are there any patterns across track types?
                """

                # Run the LLM with tool-calling capabilities
                llm_response = run_tools(prompt)

                # Display the LLM's response
                if "choices" in llm_response and len(llm_response["choices"]) > 0:
                    response_content = llm_response["choices"][0]["message"].get("content", "")
                    if response_content:
                        st.session_state.llm_response = response_content
                    else:
                        st.warning("No content returned from LLM.")
                else:
                    st.error("No valid response from LLM.")
        ...
        with st.expander("AI summary", expanded=True):
            with st.chat_message('ai'):
                st.markdown(f"{st.session_state.llm_response} \
                            \n\n**Note**: This analysis is based on available data and may not capture all biological nuances.\
                            This was generated by AI and may be incorrect."
                            )
